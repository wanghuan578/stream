...
import static com.spirit.common.constant.ResultType.UPLOAD_THREAD_POOL_FULL;
...
public class UploadVideoServciceImpl implements UploadVideoServcice {
    ...
    public int upload(final String title, final String filePath, String parentId, final TranslateBizInfo info) throws MainStageException {

        try {
            uploadTaskExecutor.execute(new Runnable() {
                public void run() {
                    info.setStatus(uploadVideo(accessKeyId, accessKeySecret, title, filePath) == true ? 5 : 4);
                    translateRepository.save(info);
                }
            });
        }
        catch (TaskRejectedException e) {
            throw new MainStageException(UPLOAD_THREAD_POOL_FULL);
        }

        return 0;
    }

    private boolean uploadVideo(String accessKeyId, String accessKeySecret, String title, String pathFile) {

        UploadVideoRequest request = new UploadVideoRequest(accessKeyId, accessKeySecret, title, pathFile);
